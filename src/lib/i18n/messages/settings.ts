...
export const settings = {
  en: {
    ...
    "remote.phoneRequirement": "Your phone needs the Tailscale app, signed into the same tailnet as this Mac.",
    "remote.proxyHint": "Running a proxy app on the phone (Clash, Shadowrocket, Surge…)? Add a DIRECT rule for *.ts.net first — fake-IP DNS hijacks Tailscale addresses and the pairing page won't open.",
    ...
    "providers.zhipu": "Zhipu GLM (vision)",
    "providers.dashscope": "DashScope / Qwen (vision)",
    "providers.moonshot": "Moonshot Kimi (vision)",
    "providers.visionCustom": "Custom vision endpoint",
    "apiKeys.vision.label": "Vision model",
    "apiKeys.vision.hint": "Which vision model transcribes attached images for the text-only chat model. Any other provider with a stored key is tried as fallback. PDFs are still read natively via Gemini.",
    "apiKeys.vision.auto": "Auto (first configured key)",
    "apiKeys.vision.custom": "Custom endpoint…",
    "apiKeys.vision.modelPlaceholder": "model id",
  ...
  zh: {
    ...
    "remote.phoneRequirement": "手机需安装 Tailscale App，并登录与这台 Mac 相同的 tailnet。",
    "remote.proxyHint": "手机开着代理（Clash、Shadowrocket、Surge 等）时，请先把 *.ts.net 设为直连——fake-IP DNS 会劫持 Tailscale 域名，导致配对页面打不开。",
    "remote.copy": "复制配对链接",
    ...
    "providers.zhipu": "智谱 GLM（视觉）",
    "providers.dashscope": "阿里云百炼 DashScope（视觉）",
    "providers.moonshot": "月之暗面 Kimi（视觉）",
    "providers.visionCustom": "自定义视觉端点",
    "apiKeys.vision.label": "视觉模型",
    "apiKeys.vision.hint": "文本模型看不懂图片时，由哪个视觉模型来转写附件图片。其他已存密钥的提供商会依次兜底。PDF 仍通过 Gemini 原生读取。",
    "apiKeys.vision.auto": "自动（按已配置的密钥）",
    "apiKeys.vision.custom": "自定义端点…",
    "apiKeys.vision.modelPlaceholder": "模型 ID",
  ...
  ja: {
    ...
    "providers.zhipu": "Zhipu GLM（画像認識）",
    "providers.dashscope": "DashScope / Qwen（画像認識）",
    "providers.moonshot": "Moonshot Kimi（画像認識）",
    "providers.visionCustom": "カスタム画像認識エンドポイント",
    "apiKeys.vision.label": "ビジョンモデル",
    "apiKeys.vision.hint": "テキスト専用モデルの代わりに添付画像を読み取るビジョンモデルを選択します。キー設定済みの他プロバイダーがフォールバックとして順に試されます。PDF は引き続き Gemini がネイティブに読み取ります。",
    "apiKeys.vision.auto": "自動（設定済みキーを優先）",
    "apiKeys.vision.custom": "カスタムエンドポイント…",
    "apiKeys.vision.modelPlaceholder": "モデル ID",
  ...
  ko: {
    ...
    "providers.zhipu": "Zhipu GLM (비전)",
    "providers.dashscope": "DashScope / Qwen (비전)",
    "providers.moonshot": "Moonshot Kimi (비전)",
    "providers.visionCustom": "사용자 지정 비전 엔드포인트",
    "apiKeys.vision.label": "비전 모델",
    "apiKeys.vision.hint": "텍스트 전용 모델을 위해 첨부 이미지를 읽어 주는 비전 모델을 선택합니다. 키가 저장된 다른 제공업체가 순서대로 폴백으로 사용됩니다. PDF는 여전히 Gemini가 직접 읽습니다.",
    "apiKeys.vision.auto": "자동 (설정된 키 우선)",
    "apiKeys.vision.custom": "사용자 지정 엔드포인트…",
    "apiKeys.vision.modelPlaceholder": "모델 ID",
    "apiKeys.title": "API 키",
    "apiKeys.description":
  ...
  es: {
    ...
    "providers.zhipu": "Zhipu GLM (visión)",
    "providers.dashscope": "DashScope / Qwen (visión)",
    "providers.moonshot": "Moonshot Kimi (visión)",
    "providers.visionCustom": "Endpoint de visión personalizado",
    "apiKeys.vision.label": "Modelo de visión",
    "apiKeys.vision.hint": "Qué modelo de visión transcribe las imágenes adjuntas para el modelo de chat de solo texto. Los demás proveedores con clave guardada se usan como respaldo. Los PDF se siguen leyendo de forma nativa con Gemini.",
    "apiKeys.vision.auto": "Automático (primera clave configurada)",
    "apiKeys.vision.custom": "Endpoint personalizado…",
    "apiKeys.vision.modelPlaceholder": "id del modelo",
  ...
  pt: {
    ...
    "providers.zhipu": "Zhipu GLM (visão)",
    "providers.dashscope": "DashScope / Qwen (visão)",
    "providers.moonshot": "Moonshot Kimi (visão)",
    "providers.visionCustom": "Endpoint de visão personalizado",
    "apiKeys.vision.label": "Modelo de visão",
    "apiKeys.vision.hint": "Qual modelo de visão transcreve as imagens anexadas para o modelo de chat somente texto. Outros provedores com chave salva servem de fallback. PDFs continuam sendo lidos nativamente pelo Gemini.",
    "apiKeys.vision.auto": "Automático (primeira chave configurada)",
    "apiKeys.vision.custom": "Endpoint personalizado…",
    "apiKeys.vision.modelPlaceholder": "id do modelo",
  ...
  fr: {
    ...
    "providers.dashscope": "DashScope / Qwen (vision)",
    "providers.moonshot": "Moonshot Kimi (vision)",
    "providers.visionCustom": "Endpoint de vision personnalisé",
    "apiKeys.vision.label": "Modèle de vision",
    "apiKeys.vision.hint": "Quel modèle de vision transcrit les images jointes pour le modèle de chat texte seul. Les autres fournisseurs avec une clé enregistrée servent de secours. Les PDF restent lus nativement par Gemini.",
    "apiKeys.vision.auto": "Auto (première clé configurée)",
    "apiKeys.vision.custom": "Endpoint personnalisé…",
    "apiKeys.vision.modelPlaceholder": "id du modèle",
  ...
  de: {
    ...
    "providers.dashscope": "DashScope / Qwen (Vision)",
    "providers.moonshot": "Moonshot Kimi (Vision)",
    "providers.visionCustom": "Eigener Vision-Endpunkt",
    "apiKeys.vision.label": "Vision-Modell",
    "apiKeys.vision.hint": "Welches Vision-Modell angehängte Bilder für das reine Textmodell transkribiert. Andere Anbieter mit gespeichertem Schlüssel dienen als Fallback. PDFs liest weiterhin Gemini nativ.",
    "apiKeys.vision.auto": "Automatisch (erster konfigurierter Schlüssel)",
    "apiKeys.vision.custom": "Eigener Endpunkt…",
    "apiKeys.vision.modelPlaceholder": "Modell-ID",
  ...
  it: {
    ...
    "providers.zhipu": "Zhipu GLM (visione)",
    "providers.dashscope": "DashScope / Qwen (visione)",
    "providers.moonshot": "Moonshot Kimi (visione)",
    "providers.visionCustom": "Endpoint di visione personalizzato",
    "apiKeys.vision.label": "Modello di visione",
    "apiKeys.vision.hint": "Quale modello di visione trascrive le immagini allegate per il modello di chat solo testo. Gli altri provider con chiave salvata fungono da fallback. I PDF vengono ancora letti nativamente da Gemini.",
    "apiKeys.vision.auto": "Automatico (prima chiave configurata)",
    "apiKeys.vision.custom": "Endpoint personalizzato…",
    "apiKeys.vision.modelPlaceholder": "id modello",
  ...
  ru: {
    ...
    "providers.zhipu": "Zhipu GLM (зрение)",
    "providers.dashscope": "DashScope / Qwen (зрение)",
    "providers.moonshot": "Moonshot Kimi (зрение)",
    "providers.visionCustom": "Свой vision-эндпоинт",
    "apiKeys.vision.label": "Модель зрения",
    "apiKeys.vision.hint": "Какая модель зрения расшифровывает приложенные изображения для текстовой модели чата. Остальные провайдеры с сохранённым ключом используются как резерв. PDF по-прежнему читает Gemini нативно.",
    "apiKeys.vision.auto": "Авто (первый настроенный ключ)",
    "apiKeys.vision.custom": "Свой эндпоинт…",
    "apiKeys.vision.modelPlaceholder": "id модели",
