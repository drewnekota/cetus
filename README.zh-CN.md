...
### 接下来的方向：多模态，因为生活本就是多模态的

今天的 Cetus 在文本上推理 —— 屏幕 OCR、转写文字、敲进去的 prompt。这是中途的一站，而不是终点。随着模型不断变强，一个既足够聪明、又足够便宜、便宜到能整天开着跑的多模态模型终将到来 —— 而押注它的理由简单得近乎朴素：**生活本身就是多模态的。** 一个人的一天是视频、是声音、是语音、是手势 —— 而不是一串 token。

所以一个真正的生活助手，必须以生活本来的方式去迎接它：接受多模态的输入，也产出多模态的输出 —— *原生地*。我们说的多模态，是真正地去*理解*视频和语音、并以对应的模态去*回应* —— 而不是我们今天还在依赖的那些权宜之计（从视频里抽帧、把语音拍平成文字）。那些只是多模态智能仍然稀缺而昂贵的当下的过渡桥梁。等它不再稀缺，桥就拆掉，agent 就直接以你本就生活其中的那些模态去感知、去行动。
