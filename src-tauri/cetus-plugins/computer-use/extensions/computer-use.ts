 ...
 * TOOLS EXPOSED
 * -------------
 *   computer_observe { app?, includeOcr?, includeScreenshot? }
...
type CuaPayload =
  | { op: "dump"; app?: string; includeOcr?: boolean; includeScreenshot?: boolean }
...
function asResult(out: unknown): ToolResult {
...
function observationResult(reply: CuaReply): ToolResult {
  let text = formatObservation(reply);
  if (reply.screenshotError) {
    text += `\n\nScreenshot was requested but unavailable: ${reply.screenshotError}`;
  }
  if (!reply.screenshotJpeg) return asResult(text);
  return {
    content: [
      {
        type: "text",
        text:
          text +
          "\n\nA current screen screenshot is attached. Use it to resolve visual " +
          "layout, icons, canvas content, or AX/OCR ambiguity. Prefer indexed " +
          "actions when available; use move_click coordinates only when there is " +
          "no reliable indexed target.",
      },
      { type: "image", data: reply.screenshotJpeg, mimeType: "image/jpeg" },
    ],
  };
}
...
const ext: Extension = (ctx: ExtensionContext) => {
  ...
  ctx.registerTool({
    ...
    description:
      ...
      "not need pixels for routine UI work; normally act by INDEX. Returns an observation_id; indices are " +
      "valid only for that observation, so re-observe after acting. Optionally " +
      "target a specific app by name or bundle id. Set includeScreenshot only " +
      "when visual layout, canvas content, icons, or AX/OCR ambiguity matter.",
    parameters: {
      type: "object",
      properties: {
        app: {
        ...
        includeScreenshot: {
          type: "boolean",
          description:
            "If true, attach the current screen as an image to this tool " +
            "result. Use sparingly because screenshots may contain sensitive " +
            "information; prefer the numbered AX list when it is sufficient.",
        },
      },
    },
    async execute(
      _toolCallId: string,
      args: { app?: string; includeOcr?: boolean; includeScreenshot?: boolean },
      _signal: unknown,
      _onUpdate: unknown,
      exCtx: ExtensionContext,
    ) {
      try {
        ...
        return observationResult(reply);
      } catch (err) {
